...
void test_positive(char * d , size_t l, const char *fname, detect_mode_t mode,
...
void test_positive(char * data, size_t l, const char *fname, detect_mode_t mode,
                   int flag_invert, int flag_true, int flag_quiet)
{
    if(l == 0){
	return;
...
test_positive(code, Size, "fuzzer", mode, flag_invert, flag_true, flag_quiet);
}
...
/*int main(int argc, const char *argv[])
{
    ...
    if (offset == argc) {
        test_positive("ssss", 100, "stdin", mode, flag_invert, flag_true, flag_quiet);
    } else {
        for (j = 0; j < flag_slow; ++j) {
            for (i = offset; i < argc; ++i) {
                    test_positive(argv[i], 10,"fuzzer", mode, flag_invert, flag_true, flag_quiet);
            }
        }
    }
